...
import { PreReservasQueueTab } from '@/components/PreReservasQueueTab'
...
export default function Reservas() {
  ...
  const [activeMainTab, setActiveMainTab] = useState<'fila' | 'pre_reservas'>('fila')
  const [pendentesPreReservaCount, setPendentesPreReservaCount] = useState<number>(0)
  ...
  const loadPendentesCount = async () => {
    if (isOperadorOrAdmin) {
      try {
        const count = await PreReservasService.countPendentes()
        setPendentesPreReservaCount(count)
      } catch (e) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Alternância Principal: Fila de Reservas vs Pré-Reservas (Solicitações) */}
      {isOperadorOrAdmin && (
        <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
          <Button
            variant={activeMainTab === 'fila' ? 'default' : 'outline'}
            size="sm"
          ...
          >
            ...
            Fila de Reservas (Convencional)
          </Button>

          <Button
            variant={activeMainTab === 'pre_reservas' ? 'default' : 'outline'}
            size="sm"
            onClick={() => {
              setActiveMainTab('pre_reservas')
              loadPendentesCount()
            }}
            className={
              activeMainTab === 'pre_reservas'
                ? 'bg-blue-600 hover:bg-blue-700 text-white gap-1.5'
                : 'gap-1.5'
            }
          >
            <Clock className="w-4 h-4" />
            Pré-Reservas (Solicitações)
            {pendentesPreReservaCount > 0 && (
              <Badge
                variant="secondary"
                className={`ml-1 text-[11px] px-1.5 py-0 h-4 font-bold ${
                  activeMainTab === 'pre_reservas'
                    ? 'bg-white text-blue-700'
                    : 'bg-rose-500 text-white'
                }`}
              >
                {pendentesPreReservaCount}
              </Badge>
            )}
          </Button>
        </div>
      ...
      {/* Visualização de Pré-Reservas */}
      {isOperadorOrAdmin && activeMainTab === 'pre_reservas' ? (
        <PreReservasQueueTab />
      ) : (
        <>
          {/* Tabs and Filter */}
          ...
          {!isOperadorOrAdmin && (
            <div className="bg-amber-50/70 border border-amber-200 rounded-lg p-3 text-xs text-amber-900 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span>
                  Você está acompanhando a fila de espera das suas reservas ativas. Quando um
                  exemplar for devolvido, a equipe da biblioteca atualizará seu pedido.
                </span>
              </div>
            </div>
          )}
                          ...
                          <div className="bg-emerald-50 border border-emerald-300 rounded-lg p-3 text-xs text-emerald-950 flex items-start gap-2.5 mt-1 shadow-xs">
                            ...
                            <div className="space-y-1 flex-1">
                              ...
                              <p className="text-[11px] text-emerald-800 leading-relaxed">
                                O livro já está separado na recepção da biblioteca. Compareça dentro
                                do prazo de reserva garantida (
                          ...
                          <div className="bg-amber-50/80 border border-amber-200/90 rounded-lg p-2.5 text-xs text-amber-900 flex items-start gap-2.5 mt-1">
                            ...
                            <div className="space-y-1 flex-1">
                              ...
                              <p className="text-[11px] text-amber-800 leading-relaxed">
                                ...
                                atualmente emprestados. Você será notificado por e-mail/push assim
                                que o livro for liberado.
                              </p>
                            </div>
                          </div>
                        )}
